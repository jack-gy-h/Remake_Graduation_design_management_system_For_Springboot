...
<head>
    ...
    <script>
        ...
        function loadData(office, topic, teacher, teacheridentitynumber, type, source) {
            $("#dg").datagrid({
                title: "审核教师申报题目信息列表",
                url: "/task/viewauditteacherassignListData",
                method: "POST",
                pagination: true,
                pageSize: 20,
                pageList: [10, 20, 30],
                ...
                columns: [[
                    ...
                    {field: 'studentname', title: '被选学生', width: 200, align: 'center'},
                    {field: 'teachername', title: '申报教师', width: 200, align: 'center'},
                    {field: 'teacherId', title: '教师编号', width: 200, align: 'center'},
                    {field: 'studentId', title: "被选学生id", width: 200, hidden: true, align: 'center'},
        ...
        function formatTitle(val, row) {
            return "<a target='_self'  style='text-decoration:none' href='/task/viewtopic?id=" + row.id + "'>查看详情</a>&nbsp;<a target='_self' style='text-decoration:none' href='/task/audit/teacherassignTask?id=" + row.id + "&audit_status=1&studentid="+row.studentId+"&teacherchoosestatus=2'>审核通过</a>&nbsp;<a target='_self' style='text-decoration:none' href='/task/audit/teacherassignTask?id=" + row.id + "&audit_status=2&studentid=" + row.studentId + "&teacherchoosestatus=3''>审核不通过</a>"
