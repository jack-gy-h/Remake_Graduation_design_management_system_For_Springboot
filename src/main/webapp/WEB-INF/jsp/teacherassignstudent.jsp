    ...
    <script type="text/javascript">
        ...
        function getThree() {//如果第一个下拉列表的值改变则调用此方法
            ...
            if (majorId != null && "" != majorId && -1 != majorId) {
                //通过ajax传入后台，把orderTypeName数据传到后端
                $.post("/data/user", {majorId: majorId,identitys:1,usergrade: '${usergrade}'}, function (data) {
                    var res = $.parseJSON(data);//把后台传回的json数据解析
                    var option = '';
                    option = '<option value="0">' + '请选择' + '</option>';
                    for (var i = 0; i < data.length; i++) {
                    ...
                    $("#studentname").html(option);//将循环拼接的字符串插入第二个下拉列表
                    $("#studentname").show();//把第二个下拉列表展示
...
<form:form id="inputForm" modelAttribute="task" action="/task/teacher/assignstudent/save" method="post" class="form-horizontal">
    <form:hidden path="id"/>
...
<%--                              class="required"/>--%>
    ...
    <div class="control-group">
        ...
        <div class="controls">
            <form:select path="studentname" class="required input-xlarge" onchange="">
                <%--            <form:options items="${UserParentMenu}" itemLabel="name" itemValue="Office" htmlEscape="false"--%>
                <%--                          class="required"/>--%>
                <%--            <form:option value="1" label="系统用户"/>--%>
            </form:select>
