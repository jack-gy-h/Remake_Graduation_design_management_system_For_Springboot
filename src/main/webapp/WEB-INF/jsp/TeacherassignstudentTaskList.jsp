...
<head>
    ...
    <script>
        ...
        function loadData() {
            $("#dg").datagrid({
                title: "教师申报题目列表",
                url: "/task/teacherassignListData",
                method: "POST",
                pagination: true,
                pageSize: 20,
                pageList: [10, 20, 30],
                ...
                columns: [[
                    ...
                    {field: 'collegename', title: '指定学生的院系', width: 200, align: 'center'},
                    {field: 'majorname', title: '指定学生的专业', width: 200, align: 'center'},
                    {field: 'studentname', title: '指定学生', width: 200, align: 'center'},
        ...
        function formatTitle(val, row) {
            if(row.teacherchoosestatusId == 1){
                return "<a target = '_self'   style = 'text-decoration:none'href = '/task/student/viewtopic?id=" + row.id + "' > 查看详情 </a>&nbsp;<a target = '_self' style = 'text-decoration:none'href='/task/teacher/modifyassigntopic?id=" + row.id + "' > 修改 </a>&nbsp;<a target = '_self' style = 'text-decoration:none'href='/task/student/delete?id=" + row.id + "' > 删除 </a>"
            }else if (row.teacherchoosestatusId == 3){
                return "<a target = '_self'   style = 'text-decoration:none'href = '/task/student/viewtopic?id=" + row.id + "' > 查看详情 </a>&nbsp;<a target = '_self' style = 'text-decoration:none'href='/task/teacher/modifyassigntopic?id=" + row.id + "' > 修改后提交 </a>"
            }
        ...
        function release() {
            ...
            window.location.href = '/task/teacher/assignstudent/form';
